...
const PostDetails = ({ user }) => {
  ...
  const fetchPostDetails = () => {
    ...
    if (query) {
      ...
      .then((data) => {
        ...
        if (data[0]) {
          const query2 = postDetailMorePostQuery(data[0])
  ...
  return (
    <>
      {postDetail && (
        <div className="flex xl:flex-row flex-col m-auto bg-white" style={{ maxWidth: '1200px', borderRadius: '32px', maxHeight: '700px' }}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex gap-2 items-center">
                <a
                  ...
                  download
                  className="bg-secondaryColor p-2 text-xl rounded-full flex items-center justify-center text-dark opacity-75 hover:opacity-100 hover:shadow-md duration-200"
                >
                  <MdDownloadForOffline />
                </a>
              </div>
              <a href={postDetail.destination} target="_blank" rel="noreferrer" className='bg-gray-100 rounded-full py-2 px-4 opacity-75 hover:opacity-100 duration-200 hover:shadow-md'>
